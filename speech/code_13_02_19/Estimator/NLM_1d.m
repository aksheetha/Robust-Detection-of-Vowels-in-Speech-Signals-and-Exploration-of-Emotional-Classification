 function [denoisedSig,debug] = NLM_1d(signal,lambda,P,PatchHW)
...
% P: max search distance(neighbour search distance)
...
for idx = Pvec  % loop over all possible differences: s-t
    ...
    SSD=zeros(size(k));    % initialising the SSD
